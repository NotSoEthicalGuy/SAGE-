...
export default function GradesPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setEnrollments(gradesData as any[]);
  ...
  // GPA comes from the student profile (`cumulativeGpa` on the Student model).
  // /student/grades only returns enrollments that already have a finalGrade,
  // so each one counts toward completed credits / courses.
  const gpa =
    typeof profile?.cumulativeGpa === 'number'
                ...
                <tbody>
                  {filteredEnrollments.map((enrollment) => (
                    <tr key={enrollment.id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-3 px-4 font-medium text-gray-900">{enrollment.course?.code}</td>
                      <td className="py-3 px-4 text-gray-900">{enrollment.course?.title || enrollment.course?.name}</td>
                      <td className="text-center py-3 px-4 text-gray-900">{enrollment.course?.credits}</td>
